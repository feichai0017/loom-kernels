...
use clap::Parser;
use quillcache_catalog::{HoltCatalog, PersistentCatalog};
use serde::Serialize;
...
#[derive(Debug, Parser)]
#[command(name = "quillcache-control")]
#[command(about = "Global catalog and scheduler for distributed attention")]
struct Args {
    #[arg(long, default_value = "127.0.0.1:8080")]
    bind: SocketAddr,
    #[arg(long, default_value = ".quillcache/catalog")]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        .with_env_filter(std::env::var("RUST_LOG").unwrap_or_else(|_| "quillcache=info".to_owned()))
        .init();
    let args = Args::parse();
    ...
    let app = Router::new()
        ...
        .with_state(state);
    tracing::info!(bind = %args.bind, "starting QuillCache control service");
